...
// FIX Bug 1: derive date string from a specific timestamp, not from "now"
...
function mixColor(ratio: number, target: number): string {
  ...
  if (diff <= 0.05) return "#34C759";
  if (diff <= 0.10) return "#FF9500";
  return "#FF3B30";
}
...
function MetricSection({
  title, m, objetivoNeto, objetivoBruto,
}: {
  title: string;
  m: PeriodMetrics;
  objetivoNeto?: number;
  ...
  return (
    <div style={{ background: "#FFFFFF", borderRadius: 20, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", border: "1px solid #F2F2F7", padding: "16px 20px", marginBottom: 12 }}>
      <div style={{ fontSize: 13, fontWeight: 600, color: "#8E8E93", marginBottom: 12, textTransform: "uppercase", letterSpacing: 0.5 }}>
        {title}
      </div>
...
export default function OpoTime() {
  ...
  // FIX Bug 3: single tick to force re-renders; liveSeconds computed directly in render
  ...
  // liveSeconds is always fresh — no stale intermediate state
  ...
  useEffect(() => {
    const fn = () => {
      if (document.visibilityState === "visible" && estado.modo) acquireWakeLock();
    };
    document.addEventListener("visibilitychange", fn);
    return () => document.removeEventListener("visibilitychange", fn);
  }, [estado.modo, acquireWakeLock]);

  // ── FIX Bug 2: day-boundary check (robust, works after device sleep / tab throttle) ──
  // Instead of looking for a midnight window, compare the session's start date to today.
  // Fires every 5 s and also after any state change. Works even if the device was asleep.
  useEffect(() => {
    const checkDayBoundary = () => {
      ...
      if (startDate === currentDate) return; // still same day — nothing to do

      // Day has changed: save the session attributed to the day it started
      const secs = Math.floor((Date.now() - estado.inicio_timestamp) / 1000);
      if (secs > 0) {
        const updated = [...sesiones, { fecha: startDate, tipo: estado.modo, segundos: secs }];
    ...
    checkDayBoundary(); // run immediately on mount / state change
  ...
  // FIX Bug 1: attribute the session to the day it *started* (inicio_timestamp),
  // not to today's date at the moment of pressing the button.
  function stopCurrentAndGet(base: Sesion[]): Sesion[] {
    if (estado.modo && estado.inicio_timestamp) {
      const secs = Math.floor((Date.now() - estado.inicio_timestamp) / 1000);
      if (secs > 0) {
        return [...base, {
          fecha: dateStrFromTs(estado.inicio_timestamp), // ← was todayStr()
          tipo: estado.modo,
          segundos: secs,
        }];
      }
    ...
    <div style={{ minHeight: "100dvh", background: "#FFFFFF", paddingBottom: 40, maxWidth: 480, margin: "0 auto" }}>
      ...
      <div style={{ padding: "0 16px", marginBottom: 8 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8, marginBottom: 10 }}>
          <button onClick={handleLectura} style={{ background: btnLectura ? "#34C759" : "#E8F8EC", color: btnLectura ? "#FFFFFF" : "#34C759", border: "none", borderRadius: 14, padding: "13px 4px", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
            Lectura
          </button>
          <button onClick={handleTest} style={{ background: btnTest ? "#1A7A35" : "#D6EFE0", color: btnTest ? "#FFFFFF" : "#1A7A35", border: "none", borderRadius: 14, padding: "13px 4px", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
            Test
          </button>
          <button onClick={handleDescanso} style={{ background: btnDescanso ? "#FF9500" : "#FFF3E0", color: btnDescanso ? "#FFFFFF" : "#FF9500", border: "none", borderRadius: 14, padding: "13px 4px", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
            Descanso
          </button>
          <button onClick={handlePausa} style={{ background: btnPausa ? "#8E8E93" : "#F2F2F7", color: btnPausa ? "#FFFFFF" : "#8E8E93", border: "none", borderRadius: 14, padding: "13px 4px", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
            Pausa
          </button>
        </div>
